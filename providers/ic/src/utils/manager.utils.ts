...
export const createManagerActor = ({identity}: {identity: Identity}): Promise<ManagerActor> => {
  return createActor<ManagerActor>({
    canisterId: EnvStore.getInstance().get().managerCanisterId,
    idlFactory: ManagerFactory,
    identity
  });
};
...
export const getDataBucket = async ({
  ...
  return getBucket<DataBucketActor | undefined>({
    identity,
...
export const getStorageBucket = async ({
  ...
  return getBucket<StorageBucketActor>({
    identity,
...
const getBucket = async <T>({
  ...
  const actor: T | undefined = bucketId
    ? await createBucketActor<T>({identity, bucketId, idlFactory})
    : undefined;
