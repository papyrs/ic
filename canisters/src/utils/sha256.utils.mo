/**
 * source: https://github.com/enzoh/motoko-sha
 */
    ...
    private func block(data : [Nat8]) {
      ...
      while (p.size() >= 64) {
        ...
        for (i in Iter.range(0, 15)) {
          j := i * 4;
          w[i] := Nat32.fromIntWrap(Nat8.toNat(p[j + 0])) << 24 | Nat32.fromIntWrap(
            Nat8.toNat(p[j + 1])
          ) << 16 | Nat32.fromIntWrap(Nat8.toNat(p[j + 2])) << 08 | Nat32.fromIntWrap(
            Nat8.toNat(p[j + 3])
          ) << 00;
        };
        var v1 : Nat32 = 0;
